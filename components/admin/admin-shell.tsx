...
const links = [
  ...
  { href: "/admin/receitas-base", label: "Receitas base", icon: BookOpen },
...
export function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800">
      ...
      <header className="flex items-center gap-3 border-b border-blue-100 bg-white px-4 py-3 shadow-sm lg:hidden">
        <button
          type="button"
          onClick={() => setMobileOpen(true)}
          aria-label="Abrir menu"
          className="rounded-md p-1.5 text-slate-700 hover:bg-coffee-50"
        >
          <Menu className="h-6 w-6" />
        </button>
        <span className="flex items-center gap-2 font-serif text-lg font-semibold text-slate-800">
      ...
      >
        <div className="flex h-[57px] items-center justify-between border-b border-slate-800 px-4">
          ...
          >
            <Building2 className="h-6 w-6 shrink-0 text-coffee-300" />
